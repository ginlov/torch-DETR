...
class PositionalEncodingSine(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        div_term = torch.exp(
            torch.arange(0, S // 2, 2, device=device).float()
            * (-torch.log(torch.tensor(10000.0, device=device)) / (S // 2))
        )
...
class Transformer(torch.nn.Module):
    ...
    def forward(
        self, src, pos_encoding, query_embedding, key_mask: torch.Tensor = None
    ):  # query embedding is learnt positional encoding as described in the DETR paper
        """
        ...
        tgt = torch.zeros_like(
            query_embedding
        )  # Initialize target with zeros as described in the DETR paper
        ...
        for layer in self.decoder:
            tgt = layer(
                tgt,
                src,
                positional_encoding=query_embedding,
                encoder_positional_encoding=pos_encoding,
                key_mask=key_mask,
            )
...
class DecoderLayer(torch.nn.Module):
    ...
    def forward(
        ...
        positional_encoding: Positional encoding tensor for the target (N, num_queries, d_model)
        encoder_positional_encoding: Positional encoding tensor for the memory (N, H x W, d_model)
