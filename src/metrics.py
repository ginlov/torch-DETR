...
def AP(
    ...
    labels and bboxes are padded to the same length of num_queries with the value
    ...
    Args:
        ...
        iou_threshold (float, optional): IoU threshold to consider a prediction as true positive.
        Defaults to 0.5.
    ...
    """
    assert (
        len(labels) == len(bboxes) == len(pred_logits) == len(pred_bboxes)
    ), "Length of labels, bboxes, pred_logits and pred_bboxes must be the same"
    all_ap = []
    for c in range(0, num_classes + 1):
        ...
        for i in range(len(labels)):
            ...
            for pred_box, score in zip(boxes_sorted, scores_sorted):
                ...
                ious = box_iou_matrix(pred_box.unsqueeze(0), gt_boxes)[
                    0
                ]  # [num_gt_class]
                best_iou, best_idx = ious.max(0)
                if (
                    best_iou >= iou_threshold
                    and not matched[gt_mask.nonzero()[best_idx]]
                ):
        ...
        precisions = torch.flip(
            torch.cummax(torch.flip(precisions, dims=[0]), dim=0)[0], dims=[0]
        )
        ...
        precisions = torch.cat(
            [torch.tensor([1.0], device=precisions.device), precisions]
        )
    ...
    if len(all_ap) == 0:
        return 0.0
    return sum(all_ap) / len(
        all_ap
    )  # subtract small value to avoid returning exactly 1.0
