...
def test_cppe5_dataset_loading(cppe5_dataset):
    ...
    for i in range(min(5, len(dataset))):  # Check first 5 samples
        ...
        ), "'boxes' is not a torch.Tensor."
        assert isinstance(
            targets["labels"], torch.Tensor
        ), "'labels' is not a torch.Tensor."
...
def test_cppe5_collate_fn(cppe5_dataset):
    ...
    assert (
        images.shape[1] == 3
    ), f"Image channel mismatch: expected 3, got {images.shape[1]}."
    ...
    ), f"Batch size mismatch in masks: expected {batch_size}, got {masks.shape[0]}."
    assert (
        masks.shape[1] == images.shape[2]
    ), f"Mask height mismatch: expected {images.shape[2]}, got {masks.shape[1]}."
    assert (
        masks.shape[2] == images.shape[3]
    ), f"Mask width mismatch: expected {images.shape[3]}, got {masks.shape[2]}."
