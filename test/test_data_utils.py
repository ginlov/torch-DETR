...
def test_visualize_output():
    ...
    out_bboxes = [torch.tensor([[0.1, 0.1, 0.2, 0.2], [0.3, 0.3, 0.4, 0.4]]),
                  torch.tensor([[0.5, 0.5, 0.6, 0.6]])]
    gt_labs = [torch.tensor([1]), torch.tensor([2, 3])]
    gt_bboxes = [torch.tensor([[0.1, 0.1, 0.2, 0.2]]),
                 torch.tensor([[0.5, 0.5, 0.6, 0.6], [0.7, 0.7, 0.8, 0.8]])]
    output = visualize_output(images, masks, image_ids, out_labs, out_bboxes, gt_labs, gt_bboxes)
    assert isinstance(output, dict)
    assert len(output) == batch_size
    for img_id, img in output.items():
        assert isinstance(img_id, int)
        assert isinstance(img, (np.ndarray, type(None)))  # img can be None if visualization fails
        if img is not None:
            assert img.shape[0] == height and img.shape[1] == width and img.shape[2] == num_channels
